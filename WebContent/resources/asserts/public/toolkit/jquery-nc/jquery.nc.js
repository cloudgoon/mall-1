...
(function(window) {
    ...
    EventManger.prototype.one = function(types, selector, data, fn) {
        ...
        this.$this.one(types, selector, data, fn);
    };
    /**
     * 输出日志
     * @return {[type]} [description]
...
Nc.loading = function(elementText, option) {
 ...
 /**
  * 对layer询问框的封装
  * sizeEnum: 窗体的大小，分为3种 large，small, extraSmall sedsamall
  */
    ...
    var asyncSubmit = Nc.asyncSubmit = function($form, options) {
        ...
        var //默认配置
            ...
            getPostData = function() {
                return options.objSerializeType ? ($.extend($form.serializeObject(), options.asyncExtData)) : (function() {
                    var a = $form.serialize(),
                        e = jQuery.param(options.asyncExtData);
                    return a ? (e ? a + "&" + e : a) : jQuery.param(options.asyncExtData);
                }());
            };
...
(function($ , win) {
    ...
    var optionsDefault = {
        ...
        areaModBtnText : '编辑' ,
        //编辑按钮的html
        areaModBtnHtml: '<a href="javascript:void(0)" class="btn btn-xs btn-default vm m-l-5"><i class="icon icon-edit"></i></a>',
        //编辑按钮及地区名称包裹的html
        areaModBtnWarp: '<div class="text-box"></div>',
        // 是否在外层就可以点击
        areaModBtnWarpClick : false ,
        //初始化的地区id
        areaModBtnAreaId: 1,
        ...
        //分类深度
        showDeep: 3,
        //当点击一个地区的时候就清空隐藏input
        //只有当末选的时候才会改变隐藏input的值
        //如果直接使用validate插件验证的话 ， 需要设置validate插件 的 ignore:'' 让他不过滤 :hidden
        //
        selectClear : false ,
        //地区请求地址
    ...
    NcArea.prototype = {
        /**
         * 重新启动
         * 使用举例：$("#goods_class").data("nc.area").restart()
         */
        restart: function() {
            var that = this;
            //当前深度
        ...
        _initAreaByAreaText: function() {
            ...
            modBtn.append(_options.areaModBtnText)
            ...
            if (_options.areaModBtnWarpClick){
                $areaModBtnWarp.one("click",clickFn );
            }else{
                modBtn.one("click",clickFn );
            }
        ...
        addAreaSelect: function(area_id) {
            ...
            $.getJSON(this.options.url + area_id, {}, function(json) {
                ...
                that.$element.append(Nc.filterScript(html ) );
        ...
        _removeSelect: function(sliceNum) {
            ...
            this.$element.children().parent().addClass('new-select');
        },
        ...
        _sendCallBack: function() {
            if (!this.options.selectClear) {
                //修改隐藏input的值
                this.inputList[0].val( this.getLast() ? this.getLast().areaId : 0 );
 ...
 */
(function($) {
    var BrowseAndGuessLike = function(element, option) {
        //初始化元素
        this.$element = $(element);
        this.options = $.extend({}, BrowseAndGuessLike.setting, option);
        this.init();
    };
    //默认设置
    BrowseAndGuessLike.setting = {
        showNum: 4
    };
    BrowseAndGuessLike.prototype = {
        init: function() {
            var that = this;
            //加载
            that._loadBrowseAndGuessLike(true);
        },
        //加载记录
        _loadBrowseAndGuessLike: function() {
            var that = this;
            $.ajax({
                url: ncGlobal.webRoot + 'goods/browse/and/like',
                success: function(html) {
                    that.$element.html(html);
                    var numGuessLike = $('#guessLikeList').find("li").length;
                    (numGuessLike > 0 && numGuessLike > that.options.showNum) &&  $('#guessLikeList').bxSlider({
                        infiniteLoop: false,
                        hideControlOnEnd: true,
                        hideHoverControls:true,
                        minSlides: 0,
                    ...
                    var numBrowse = $('#goodsBrowseList').find("li").length;
                    (numBrowse > 0 && numBrowse > that.options.showNum) &&  $('#goodsBrowseList').bxSlider({
                        infiniteLoop: false,
                        hideControlOnEnd: true,
                        hideHoverControls:true,
                        minSlides: 0,
    ...
    //多转单
    function Plugin(option) {
        return new BrowseAndGuessLike(this, option);
    }
    //jquery 绑定
    $.fn.ncBrowseAndGuessLike = Plugin;
})(jQuery);
...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        // Node/CommonJS Copyright: www.Bizpower.com
    ...
    var config = $.cookie = function (key, value, options) {
        ...
        // Write Copyright: Bizpower多用户商城系统
...
(function (global, factory) {
    ...
    function defaultParsingFlags() {
        // We need to deep clone this object. Copyright: www.bizpower.com
        return {
            empty           : false,
            unusedTokens    : [],
            unusedInput     : [],
    ...
    function configFromStringAndFormat(config) {
        ...
        // add remaining unparsed input length to the string Copyright: www.bizpower.com
...
(function (global, factory) {
    ...
    // from http://docs.closure-library.googlecode.com/git/closure_goog_date_date.js.source.html Copyright: www.bizpower.com
    // somewhat more in line with 4.4.3.2 2004 spec, but allows decimal anywhere
    var create__isoRegex = /^(-)?P(?:(?:([0-9,.]*)Y)?(?:([0-9,.]*)M)?(?:([0-9,.]*)D)?(?:T(?:([0-9,.]*)H)?(?:([0-9,.]*)M)?(?:([0-9,.]*)S)?)?|([0-9,.]*)W)$/;
...
(function($) {
    ...
    Cart.setting = {
        ...
        tpl: {
            ...
            //顶部每个商品的模板
            goodsItem: '<div class="goods-list">'            
            + '<div class="goods-thumb">'
            + '<a target="_blank" href="' + ncGlobal.webRoot + 'goods/<%=chainId > 0 ? realCommonId : commonId%>" title="<%=goodsName%> <%=goodsFullSpecs%>">'
            + '<img src="<%=imageSrc%>"></a>'
            + '</div>'
			+ '<div class="goods-name">'
            + '<a target="_blank" href="' + ncGlobal.webRoot + 'goods/<%=chainId > 0 ? realCommonId : commonId%>" title="<%=goodsName%> <%=goodsFullSpecs%>"><%=goodsName%><p><%=goodsFullSpecs%></p></a>'
            + '</div>'
            + '<div class="goods-sales"></div>'
            + '<div class="goods-price"><%=goodsPrice%>×<%=buyNum%>'
            + '</div>'
            + '<div class="handle"><a href="javascript:void(0);" data-cart-del="<%=cartId%>">删除</a>'
            + '</div>'
            + '</div>',
            //顶部购物车 价格区域模板
            pricePanel: '<span class="total-price">共<strong>{goodsCount}</strong>种商品  共计：<strong>{amount}</strong></span>' +
            '<a href="' + ncGlobal.webRoot + 'cart/list" class="btn-cart" >去购物车</a >'
        },
        //购物车显示数量的element
        ...
        _buildGoodsList: function(data) {
            var that = this,
                a = data.goodsList.map(function(currValue) {
                    ...
                    currValue.goodsPrice = Nc.priceFormatNew(currValue.goodsPrice);
            ...
            this.$goodsPrice.html(this.options.tpl.pricePanel.ncReplaceTpl({
                ...
                amount: Nc.priceFormatNew(data.amount)
            })).show();
...
(function (window, $) {
        ...
        CountDown.prototype._createTimer = function () {
            ...
            this._timer = setInterval(function () {
                var _zero = "0"
                if (that.time > that.options.stopSencode) {
...
(function (window) {
    ...
    window.ncImage = function ncImage(imageName, width, height) {
        ...
        if (type == ".jpg" || type == ".gif" || type == ".png") {
            ...
            //新版OSS接口切图规则
            //return imageName + "?x-oss-process=image/resize,w_" + width +  ",h_" + height;
        } else {
            return imageName;
        }
    };
        ...
        bindEvents: function() {
            ...
            this.$btnOff.click(function(e,isPost) {
                ...
                that.options.postUrl != "" && (
                    ...
                    //? (isPost || that._post("off"))
                    //: that._setSelectOnOff("off")Copyright: www.Bizpower.com
            })
        },
...
(function(window) {
	...
	window.ncTemplate || (window.ncTemplate = function(text) {
		...
		var template = function(data , scope) {
			return render.call(this, data, scope||Nc);
		};
 ...
 * @returns {number}
 */
Nc.jsonLength = function(jsonData){
    var jsonLength = 0;
...
(function ($, win) {
    ...
    NcArea = function (element, option) {
        ...
        this.autoGeoFinish = false
        this.geoProcessedArray = []
        this.$modBtn = []
        this.curRegeocoder = null
    ...
    var optionsDefault = {
        ...
        selectTpl: '<select id="{id}">{content}</select>',
        //option 元素模板
        optionTpl: ' <option value="{key}" data-area-code="{areaCode}">{value}</option>',
        //数据格式
        dataFormat: "areaList",
        //key
        dataIdFormat: "areaId",
        ...
        enableMap: false,
        enableInitGeo :true ,
        // 地图的key ， 如果这个是空的话就关闭地图定位
        mapKey: '',
        // 地区定位显示区域， 如果这个是空的话就关闭地图定位
        mapElement: '',
        // 详细地址填写的input 框
        addressInputElement: '',
    ...
    NcArea.prototype = {
        ...
        initMap: function () {
            console.log('initMap')
            // 判断是否开启地图定位
            var self = this, _opts = this.options, roll = Nc.randomString(8)
            if (!(_opts.enableMap && _opts.mapKey && _opts.mapElement && _opts.mapElement.length > 0)) {
                _opts.enableMap = false
                return
            }
            // 添加地图底部
            this.mapFooterEl = $('<p class="map-footer" style="display: none;"> <a href="javascript:;" class="save-map">保存位置</a> <a href="javascript:;" class="back-map">回原位置</a> </p>')
            $("#" + _opts.mapElement).after(this.mapFooterEl)
            // 初始化地址input 元素
            this.$addressInput = $("#" + _opts.addressInputElement)
            ...
            $.getScript('https://webapi.amap.com/maps?v=1.3&key=' + _opts.mapKey + '&callback=amapCallBack_' + roll + '&&plugin=AMap.Scale,AMap.PolyEditor,AMap.Geocoder,AMap.Autocomplete,AMap.PlaceSearch,AMap.InfoWindow,AMap.ToolBar')
        },
        /**
         * 生成地图
         */
        genMap: function () {
            ...
            ;
            // 生成地图实例
            this.mapObj = new AMap.Map(_opts.mapElement, _opts.mapSetting)
            //自动完成
            if (this.$addressInput.length) {
                this.auto = new AMap.Autocomplete({
                    input: _opts.addressInputElement,
                    citylimit: false
                });
                ...
                AMap.event.addListener(this.auto, "select", function (e) {
                    ...
                    // 关键字查询查询
                    self.search(e.poi.name, e.poi.adcode, function (result) {
                        self.genGeoInfo(result.poiList.pois[0].adcode)
                    });
            ...
            if (_opts.enableInitGeo){
                // 添加标记
                this.addMarker(this.mapObj.getCenter(), "", true)
                // 定位
                this.geolocation()
            }
            // 加载标尺
            ...
            this.bindMapEvent()
            //
            this.$element.trigger('nc.area.map.created')
        ...
        bindMapEvent: function () {
            var self = this
            // 绑定平移地图事件，使marker始终居中
            var mapmove = false;
            this.mapObj.on("mapmove", function (e) {
                console.log('平移地图，使marker始终居中')
                if (self.marker) {
                    // 关闭窗体
                    if (self.infoWindow) {
                        self.infoWindow.close();
                    }
                    // self.marker.setIcon("/images/common/location.png");
                    self.marker.setPosition(self.mapObj.getCenter());
                }
                mapmove = true;
            });
            // 绑定鼠标抬起事件
            this.mapObj.on("mouseup", function (e) {
                console.log('mouseup')
                if (self.marker && mapmove) {
                    self.regeocoder(self.marker.getPosition(), function (address) {
                        self.curRegeocoder = this.regeocode
                        self.openWindow(address);
                    });
                }
                if (mapmove) {
                    self.mapFooterEl.show()
                }
                // 标识平移状态：false
                mapmove = false;
                return false;
            ...
            // 地图底部保存点击
            $("body").off('click' , ".save-map").on("click", ".save-map", function () {
                console.log('保存')
                self.position = self.marker.getPosition();
                self.genGeoInfo(self.curRegeocoder)
                // 隐藏
                self.mapFooterEl.hide();
            });
            //地图底部返回按钮点击
            $("body").off('click' , ".back-map").on("click", ".back-map", function () {
                console.log('返回圆点')
                if (self.position) {
                    self.addMarker(self.position);
                }
                // 隐藏
                self.mapFooterEl.hide();
            });
            // 监听手动修改地址事件
            // 根据手动修改的地址，改变地图导航
            this.$element.off('nc.area.mapsearch')
                .on("nc.area.mapsearch", function (e, content) {
                    console.log('监听手动修改地址事件 , content is ', content)
                    if (!content) return
                    // 地图定位
                    self.search(content.replace(" ", ""), '');
                });
            this.$element.off('nc.area.restart.map').on('nc.area.restart.map' , function () {
                self.initMap()
            })
            this.$addressInput.blur(function () {
                console.log('addressinput blur')
                self.$element.triggerHandler('nc.area.mapsearch' , $.trim(self.$addressInput.val()) )
            })
        },
        /**
         * 定位
        ...
        geolocation: function () {
            ...
            this.mapObj.plugin('AMap.Geolocation', function () {
                ...
                AMap.event.addListener(geolocation, 'complete', function (result) {
                    console.log(result)
                    var address = result.formattedAddress, position = self.mapObj.getCenter()
                    self.setCity(result.addressComponent.adcode);
                    // 给地图添加点标记等覆盖物
                    self.addMarker(position, address)
                    self.genGeoInfo(result)
                });

                //返回定位出错信息
                AMap.event.addListener(geolocation, 'error', function (e) {
                    console.log('无法获取位置信息，请检查定位权限是否开启')
                });
            })
        },
        /**
         * 生成自用的位置信息
         */
        genGeoInfo: function (geocoder) {
            console.log('genGeoInfo')
            var self = this,
                isOnlyAdcode = typeof geocoder === 'string' ? true : false,
                handleGeocoder = function () {
                    var adcode = geocoder.addressComponent.adcode,
                        formattedAddress = geocoder.formattedAddress
                    ;
                    self.addressComponent = geocoder.addressComponent
                    // 整理地址
                    self.addressComponent.areaCode0 = adcode.substring(0, 2) + '0000'
                    self.addressComponent.areaCode1 = adcode.substring(0, 4) + '00'
                    self.addressComponent.areaCode2 = adcode
                    self.geoProcessedArray = []

                    // 关联的input中写上详细地址
                    self.$addressInput.val(formattedAddress)
                },
                handleOnlyAdcode = function () {
                    var adcode = geocoder
                    self.addressComponent = {}
                    // 整理地址
                    self.addressComponent.areaCode0 = adcode.substring(0, 2) + '0000'
                    self.addressComponent.areaCode1 = adcode.substring(0, 4) + '00'
                    self.addressComponent.areaCode2 = adcode
            ...
            isOnlyAdcode ? handleOnlyAdcode() : handleGeocoder()
            this.$modBtn.length &&this.$modBtn.triggerHandler('click')
            this.$element.triggerHandler("nc.area.success", 0);
        },
        /**
         * 添加地图标记
         */
        addMarker: function (position, address, is_address) {
            ...
            if (address) {
                ...
                this.regeocoder(this.marker.getPosition(), function (address) {
                    var regeocode = this.regeocode
                    self.genGeoInfo(regeocode)
                    self.openWindow(address);
                });
            }
        ...
        search: function (name, city, callback) {
            var self = this, _opts = this.options, offset = $(_opts.mapElement).offset();
            ...
            // 关键字查询查询
            this.placeSearch.search(name, function (status, result) {
                ...
                if (result.poiList) {
                    ...
                    self.addMarker(position, poi.address + poi.name);
                } else {
                    var position = self.mapObj.getCenter();
                    console.log('未找到收货地址')
                    self.addMarker(position, "未找到收货地址，请拖动地图标注一个准确位置", false);
                }
        ...
        restart: function (setting) {
            ...
            if (this.options.enableMap ) {
                console.log('发送已选地区定位事件' ,setting.defaultGeoText)
                if (setting.defaultGeoText){
                    this.$element.off('nc.area.map.created').on('nc.area.map.created' , function () {
                        console.log('nc.area.map.created')
                        self.$element.triggerHandler('nc.area.mapsearch' , setting.defaultGeoText )
                    })
                }
                this.mapObj && (  this.mapObj.destroy( ), this.genMap() )
            }
        },
        ...
        _initAreaByAreaText: function () {
            ...
            if (_options.areaModBtnWarpClick) {
                ...
                this.$modBtn =$areaModBtnWarp.one("click", clickFn);
            } else {
                this.$modBtn =modBtn.one("click", clickFn);
            }
            //修改隐藏input的值
            this._setHiddenInput(_options.areaModBtnAreaId, _options.initAreaText);
        },
         ...
         * 添加select 元素
         */
        addAreaSelect: function (area_id, isAuto) {
            ...
            // 如果不是地图自动的选择地址就发送一个事件
            !isAuto && that.$element.triggerHandler("nc.area.mapsearch", this.getAllName());


            if (this.ajaxFlat === false) {
                return;
            } else {
                this.ajaxFlat = false;
            }
            ...
            $.getJSON(this.options.url + area_id, {}, function (json) {
                ...
                that.options.enableMap && isAuto && that.$element.triggerHandler("nc.area.success", that.$$area_deep_id);
            });
        },
        //创建一个select
        _buildHtml: function (list) {
            ...
            $.each(list, function (i, n) {
                var nk = 'undefined' != typeof n[that.options.dataIdFormat] ? n[that.options.dataIdFormat] : 0,
                    nn = 'undefined' != typeof n[that.options.dataNameFormat] ? n[that.options.dataNameFormat] : 0,
                    areaCode = n.areaCode ? n.areaCode : 0,
                    dp = 'undefined' != typeof n.deep ? n.deep : 0;
        ...
        _initEvent: function () {
            var that = this
            //每个select 绑定事件
            this.$element.on("change", "select", function (e, isAuto) {
                console.log(isAuto)
                var selectThis = $(this),
                    id = selectThis.val(),
                    area_deep_id = selectThis.data("area_deep_id"),
                ...
                that.$$area_deep_id = area_deep_id + 1;
                //添加一个新的select
                that.addAreaSelect(id, isAuto)

            })
            // 监听自动定位
                .on('nc.area.success', function (e, id) {
                    console.log('监听自动定位')
                    var target = $("#" + that.$elementId + "_nc_select_" + id)
                    // 如果处理过了就不再处理
                    if (that.geoProcessedArray.indexOf(id) >= 0) return

                    var code = that.addressComponent['areaCode' + id],
                        el = target.find('option[data-area-code=' + code + ']')
                    if (el.length) {
                        target.val(el.attr('value')).trigger('change', true)
                        el.triggerHandler("click")
                        that.geoProcessedArray.push(id)
                    }
                })
...
(function (window, $) {
    ...
    var defaultOption = {
        ...
        // 允许选框缩放 bool 默认 true
        allowResize: true,
        //设置选框 数组 默认[0,0,img[0].naturalWidth,img[0].naturalHeight]
        setSelect: [0, 0, 600, 600],
        //上传图片最小尺寸限制上传图片最小尺寸限制
        sizeLimite: "707*500",
        // 原始图片的最大值，0则不限制 ,单位之kb
        picSizeMax : 0, 
        //成功回调方法
        success: null,
        //失败回调方法
        error: null,
        //裁切图片对话框
        tplDialog: '<div class="content-cut-img"><div class="img"><img id="imgCutUpload" src="javascript:;"></div></div>',
        // 上面的id
        tagName: '#imgCutUpload' ,
        // 上传链接地址
        uploadUrl : ''
    }
    /**
     ...
     */
    function verifyDep() {
        if (!$.isFunction(Nc.layerOpen)) {
            console.error("Nc.layerOpen is undefined")
            return
        }


        if (!$.isFunction(Nc.alertError)) {
            console.error("Nc.alertError is undefined")
            return
        }
        if (!$.isFunction($.Jcrop) ) {
            console.log("$.Jcrop is undefined")
            return
        }
        return true
    }
    ...
    var CutUpload = function (el, options) {
        if (!verifyDep()) return
        this.el = $(el)
        ...
        this.layerEl = null
        this.layerIndex = 0
        this.htmlStr  = this.el.html()
        this.picSizeMax = this.opts.picSizeMax * 1024
        this.init()
    }
    /**
     *
     */
    CutUpload.prototype.init = function () {
        this.initEvents()
    }
    /**
     * 初始化事件
     */
    CutUpload.prototype.initEvents = function () {
        var self = this
        //input 上传选择文件事件
        this.el.on('change', function () {
            console.log('cut pircure change')
            self.transImgBase64()
        });
    }
    /**
     * 将图片装成base64
     */
    CutUpload.prototype.transImgBase64 = function () {
        var self = this,
        ...
        if (this.picSizeMax > 0 ){
            if(this.picSizeMax < file.files[0].size ){
                Nc.alertError("上传的图片不能大于" + _opts.picSizeMax + "KB!")
                self.resetInput()
                return 
            }
        }
        ...
        reader.onload = function (e) {
            ...
            img.onload = function () {
                ...
                var orgImgWidth = img.naturalWidth;
                var orgImgHeight = img.naturalHeight;
                var radio = orgImgWidth / orgImgHeight;
                var limiteWidth = _opts.sizeLimite.split("*")[0];
                var limiteHeight = _opts.sizeLimite.split("*")[1];
                var allowResize = typeof _opts.allowResize != 'undefined' ? _opts.allowResize : true;
                var setSelect;
                if (_opts.setSelect) {
                    setSelect = _opts.setSelect;
                } else {
                    setSelect = [0, 0, orgImgWidth, orgImgHeight];
                }
                if (orgImgWidth < limiteWidth || orgImgHeight < limiteHeight) {
                    Nc.alertError("上传的图片尺寸不小于" + _opts.sizeLimite + "!")
                    self.resetInput()
                } else {
                    _opts.url = url;
                    _opts.radio = radio;
                    _opts.limiteWidth = limiteWidth;
                    _opts.limiteHeight = limiteHeight;
                    _opts.orgImgWidth = orgImgWidth;
                    _opts.orgImgHeight = orgImgHeight;
                    _opts.allowResize = allowResize;
                    _opts.setSelect = setSelect;
                    self.showCutDialog()
                }
            }
        }
     ...
     * 显示裁切对话框
     */
    CutUpload.prototype.showCutDialog = function () {
        var self = this
        Nc.layerOpen({
            title: "裁切图片",
            content: self.opts.tplDialog,
            sizeEnum: 'small',
            yes: function () {
                self.uploadImage()
            },
     ...
     * 生成Jcrop
     */
    CutUpload.prototype.genJcrop = function () {
        var self = this,
            _el = this.el,
            _opts = this.opts,
            img = this.layerEl.find(_opts.tagName);

        img.attr("src", _opts.url);
        //重新定位layer 弹出框
        img.load(function () {
            $(window).trigger('resize')
        })
        if (self.imgJcrop) {
            self.imgJcrop.destroy();
        }
        img.css({
            'width': _opts.orgImgWidth + 'px',
            'height': _opts.orgImgHeight + 'px'
        });
        self.imgJcrop = $.Jcrop(_opts.tagName, {
            allowResize: _opts.allowResize,
            aspectRatio: _opts.aspectRatio,
            boxWidth: 300,
            boxHeight: Math.round(300 / _opts.radio),
            minSize: [_opts.limiteWidth, _opts.limiteHeight],
            setSelect: _opts.setSelect,
            bgColor:_opts.bgColor ? _opts.bgColor : '#ffffff',
            onChange: function (e) {
                $(".jcrop-keymgr").css("opacity", 0);
                self.imgData.imgFile = _el[0].files[0];
                self.imgData.cutX1 = Math.round(e.x);
                self.imgData.cutY1 = Math.round(e.y);
                self.imgData.cutWidth = Math.round(e.w);
                self.imgData.cutHeight = Math.round(e.h);
                if (!_opts.allowResize) {
                    self.imgData.cut_width = _opts.sizeLimite.split("*")[0];
                    self.imgData.cut_height = _opts.sizeLimite.split("*")[1];
                }
            }
        });
     ...
     * 上传裁切图片数据
     */
    CutUpload.prototype.uploadImage = function () {
        var self = this , formData = new FormData();

        formData.append("filedata", this.imgData.imgFile);
        formData.append("cutX1", this.imgData.cutX1);
        formData.append("cutY1", this.imgData.cutY1);
        formData.append("cutWidth", this.imgData.cutWidth);
        formData.append("cutHeight", this.imgData.cutHeight);
        $.ajax({
            ...
            success: function (xhr) {
                console.log('上图图片接收服务器返回数据...')
                layer.close(self.layerIndex)
                if (xhr.code == 200 ) {
                    self.opts.success && self.opts.success(xhr);
                } else {
     ...
     * 重置上传input
     */
    CutUpload.prototype.resetInput = function () {
    ...
    function Plugin(option) {
        return this.each(function () {
            var $this = $(this);
            var data = $this.data('cutupload');
            if (!data) $this.data('cutupload', (data = new CutUpload(this, option)))
        })
    }
